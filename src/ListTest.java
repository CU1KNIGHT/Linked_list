...
public class ListTest {
    ...
    private void appendTestElements() {
        String[] values = new String[] {"1", "3", "4", "3", "5"};
    ...
    public void testRemoveReturnNullOnEmptyList() {
        assertNull("List.remove() should return null if the list is empty!", list.remove("removeMe"));
    }

    @Test
    public void testRemoveReturnNull() {
        appendTestElements();
        assertNull("List.remove() should return null if the List contains no element with a given content!", list.remove("removeMe"));
    }

    @Test
    public void testRemoveReturnContentOnSuccess() {
        appendTestElements();
        assertEquals("List.remove() should return the removed element's content.", "3", list.remove("3"));
    }
    ...
    public void testRemoveRemovesOnlyOneOccurrence() {
        appendTestElements();
        list.remove("3");
        assertEquals("List.remove() should only remove one occurrence of a given content!", "3", list.remove("3"));
        assertNull("Also, List.remove() should then actually remove the element!", list.remove("3"));
    }

    @Test
    public void testRemoveLastElement() {
        list.append("1");
        assertEquals("List.remove() should return the removed element's content. Having only one element in the list might need to be regarded as a special case!",
        "1", list.remove("1"));
        assertNull("List.remove() should return null if the last element was removed! If you've made it to this message, make sure you actually remove thigs.", list.remove("1"));
    }
}
